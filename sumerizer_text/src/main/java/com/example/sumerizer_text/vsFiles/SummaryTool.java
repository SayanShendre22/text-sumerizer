...
public class SummaryTool{
	...
	public void init(){
		...
		try {
	        in = new FileInputStream("../sumerizer_text/src/main/java/samples/input.txt");
	        out = new FileOutputStream("../sumerizer_text/src/main/java/samples/output.txt");
	...
	public void createSummary(int n){

	      for(int j=0;j<=noOfParagraphs;j++){
	      		int primary_set = paragraphs.get(j).sentences.size()/n; 
...
//			System.out.println(sentence.value);
			Sumerized = Sumerized+sentence.value;
		}
		return Sumerized;
	}
